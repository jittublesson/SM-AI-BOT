  ...
  return (
    <div className="flex flex-col space-y-4">
      ...
      <div className="flex-1">
        ...
        {activeTab === "overview" && (
          <div className="space-y-4">
            ...
            <div className="glass-card p-4 rounded-xl">
              ...
              {breadth?.data_available === false ? (
                <div className="text-center py-4">
                  <AlertTriangle className="w-5 h-5 text-brand-muted mx-auto mb-1.5" />
                  <p className="text-[10px] text-brand-muted font-mono">{breadth?.note || "Data unavailable"}</p>
                </div>
              ) : (
                <div className="space-y-2">
                  ...
                  <div className="h-3 rounded-full overflow-hidden flex bg-black/5 dark:bg-white/5">
                    <div className="h-full bg-brand-secondary rounded-l-full" style={{ width: `${((breadth?.advances || 0) / ((breadth?.advances || 1) + (breadth?.declines || 0) + (breadth?.unchanged || 0))) * 100}%` }} />
                    <div className="h-full bg-slate-400/30" style={{ width: `${((breadth?.unchanged || 0) / ((breadth?.advances || 0) + (breadth?.declines || 0) + (breadth?.unchanged || 1))) * 100}%` }} />
                    <div className="h-full bg-brand-danger rounded-r-full" style={{ width: `${((breadth?.declines || 0) / ((breadth?.advances || 0) + (breadth?.declines || 1) + (breadth?.unchanged || 0))) * 100}%` }} />
                  </div>
                  <div className="flex gap-4 text-[10px] text-brand-muted font-mono">
                    <span>52W Highs: <strong className="text-brand-secondary">{breadth?.new_highs_52w || 0}</strong></span>
                    <span>52W Lows: <strong className="text-brand-danger">{breadth?.new_lows_52w || 0}</strong></span>
            ...
            <div className="glass-card p-4 rounded-xl">
              ...
              {!data?.ipo_calendar || data.ipo_calendar.length === 0 ? (
                <div className="text-center py-4">
                  <AlertTriangle className="w-5 h-5 text-brand-muted mx-auto mb-1.5" />
                  <p className="text-[10px] text-brand-muted font-mono">{data?.calendar_note || "No upcoming IPO data available."}</p>
                </div>
              ) : (
                <div className="space-y-2">
          ...
          <div className="space-y-3">
            {(!data?.sector_performance || data.sector_performance.length === 0 || data.sector_performance.some((s: any) => s.data_available === false)) ? (
              <div className="glass-card p-6 text-center rounded-xl">
                ...
                <h4 className="text-xs font-black text-slate-800 dark:text-white mb-1">Sector performance unavailable</h4>
                <p className="text-[10px] text-brand-muted font-mono max-w-md mx-auto">
                  {data?.sector_performance?.[0]?.note || "Real-time sector performance is not available on the free Yahoo Finance tier."}
                </p>
              </div>
            ) : (
              <>
        ...
        {activeTab === "flows" && (
          <div className="space-y-4">
            {data?.fii_dii_flows?.data_available === false ? (
              <div className="glass-card p-6 text-center rounded-xl border-l-4 border-brand-warning">
                <AlertTriangle className="w-6 h-6 text-brand-warning mx-auto mb-2" />
                <h4 className="text-xs font-black text-slate-800 dark:text-white mb-1">FII / DII flows data unavailable</h4>
                <p className="text-[10px] text-brand-muted font-mono max-w-md mx-auto">
                  {data?.fii_dii_flows?.note || "Real-time FII/DII daily transaction flows require access to direct institutional feeds."}
                </p>
              </div>
            ) : (
              (() => {
        ...
        {activeTab === "calendar" && (
          <div className="space-y-3">
            {!data?.economic_calendar || data.economic_calendar.length === 0 ? (
              <div className="glass-card p-6 text-center rounded-xl">
                <AlertTriangle className="w-6 h-6 text-brand-muted mx-auto mb-2" />
                <h4 className="text-xs font-black text-slate-800 dark:text-white mb-1">Economic calendar unavailable</h4>
                <p className="text-[10px] text-brand-muted font-mono max-w-md mx-auto">
                  {data?.calendar_note || "Economic calendar events require integration with a paid financial calendar provider."}
                </p>
              </div>
            ) : (
              <>
                ...
                {data?.commodities?.map((c: any, i: number) => {
                  ...
                  return (
                    <div key={i} className={`glass-card p-3.5 rounded-xl border ${isUp ? "border-brand-secondary/10" : "border-brand-danger/10"}`}>
                      ...
                      {c.data_available === false ? (
                        <div className="text-[9px] text-brand-muted italic mt-0.5">Unavailable</div>
                      ) : (
                        <div className={`text-[10px] font-bold font-mono mt-0.5 flex items-center gap-0.5 ${isUp ? "text-brand-secondary" : "text-brand-danger"}`}>
        ...
        {data?.data_source && (
          <div className="mt-4 p-2 rounded-lg bg-slate-50 dark:bg-slate-900 border border-light-border dark:border-dark-border text-[9px] font-mono text-brand-muted text-center flex items-center justify-center gap-1.5">
            <span className="inline-block w-1.5 h-1.5 rounded-full bg-brand-secondary animate-pulse" />
            <span>Source: <strong>{data.data_source}</strong></span>
            {data.last_updated && <span>· Updated: <strong>{data.last_updated}</strong></span>}
          </div>
        )}
      </div>
    </div>
